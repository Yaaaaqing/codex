...
class DeviceWorkflowService {
  ...
  createDevice(payload, role, user) {
    ...
    const device = {
      ...
      programVersions: [{
        ...
        uploadedBy: user, uploadedAt: now, note: 'baseline', sealed: false,
      }],
      changeLogs: [],
      inspectionTask: null,
    };
  ...
  submitInspection(deviceSn, checklistCodes, role, user) {
    this.assertRole(role, ['debugger', 'owner', 'admin']);
    const device = this.getDevice(deviceSn);
    if (device.status !== DeviceStatus.DEBUGGING) throw new WorkflowError('invalid status');
    if (!device.programVersions.some(v => v.versionType === VersionType.BASELINE)) throw new WorkflowError('baseline required');
    if (device.changeLogs.some(l => (new Date(l.loggedAt) - new Date(l.changedAt)) / 36e5 > 24)) throw new WorkflowError('overdue logs found');
  ...
  tryPromote(device) {
    ...
    if (passed && finalSealed && device.documentsReady) device.status = DeviceStatus.INSPECTED;
  }
}
